...
for item in range(test_case) :
    ...
    beauty_count = []
    start = 0
    no_of_days = int(number/2)
    while True:
        beauty_count.append(sum(list(map(int,number_set[start:start + no_of_days]))))
        if number_set[start + no_of_days] == number_set[-1] :
            break
        start += 1
    result.append(max(beauty_count))  
for item in range(len(result)) :
    print ("Case #{0}: {1}".format(item + 1,result[item]))
